\
    ...
    # Helper: sanitize a string to be a safe ID for topics/unique_ids
    sanitize_id() {
      echo "$1" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/_/g'
    }

    # Helper: publish MQTT discovery config for a camera (only once)
    ...
    # Helper: publish a single JSON event to per-camera topics
    ...
    # Helper: publish motion pulse (ON then OFF after 5s)
